...
(function init(){ 
    ...
    const url = 'https://bomba.promodo.digital/game.php';
    const urlFriendEmail = 'https://bomba.promodo.digital/validate.php';
    const form = document.querySelector('[form]');
    const formFriend = document.querySelector('[form-friend]');
    const attemptButton = document.querySelectorAll('[attempt-button]');
    ...
    const showOnAttempt1    = document.querySelectorAll('[show-on-attempt1]');
    const hideOnAttempt1    = document.querySelectorAll('[hide-on-attempt1]');
    const showOnAttempt2    = document.querySelectorAll('[show-on-attempt2]');
    const hideOnAttempt2    = document.querySelectorAll('[hide-on-attempt2]');

    console.log(showOnAttempt)
    ...
    form.addEventListener('submit', (e) => {
        e.preventDefault();
        form.querySelector("[sendemail]").setAttribute("disabled", "disabled");
        form.querySelector("[sendemail]").innerHTML = "load...";
        ...
        postData(url, formData).then((data) => {
            ...
            //current++;
            //rouletteStart(data)
            elementsShow(showOnAttempt, 0)
            elementsHide(hideOnAttempt, 0)

            validateFriendEmail(data);

            attemptButton.forEach(button=>{
    ...
    const validateFriendEmail = (data)=>{
        formFriend.addEventListener('submit', (e) => {
            e.preventDefault();
            formFriend.querySelector("[sendemail]").setAttribute("disabled", "disabled");
            formFriend.querySelector("[sendemail]").innerHTML = "load...";
            const formData = new FormData(form);
            console.log(formData.get('email'))
            postData(urlFriendEmail, formData).then((dataValidate) => {
                console.log("dataValidate response", dataValidate);
                current++;
                rouletteStart(data);
            });
        });
    ...
    const rouletteStart = (data)=>{
        ...
        if(data.attempt===0){
            if(current===attemptsMax){
                console.log("LOSS", current, data.attempt)
                loss();
            }else{
                console.log("another", current, data.attempt)
                attempt();
            }
        }else{
            if(data.attempt===current){
                console.log("WIN", current, data.attempt)
                win(data);
            }else{
                console.log("another", current, data.attempt)
                attempt();
    ...
    const attempt = ()=>{
        console.log("current attempt", current)
        rouletteAnimate(prizesList.loss[0])
        if(current===1){
            elementsShow(showOnAttempt1)
            elementsHide(hideOnAttempt1) 
        }
        if(current===2){
            elementsShow(showOnAttempt2)
            elementsHide(hideOnAttempt2) 
        }
    }
    const win = (data)=>{
        ...
        console.log(prizesImg[data.prize])
        document.querySelector('.roulette__img').setAttribute('src', prizesImg[data.prize])
    ...
    const rouletteAnimate = (prize)=>{
        ...
        let angle = getRandomArbitrary(1,4) - 1/prizesList.count*(prize-1) - 1/16;
        rouletteDisc.style.transform = 'rotate('+angle+'turn)';
        rouletteDisc.style.transition = animSpeed+'s';
    }

    const elementsHide = (elements, delay=1)=>{
        setTimeout(()=>{
            elements.forEach(item=>{
                item.setAttribute(hiddenAttr,"")
            })
        }, animSpeed*1000*delay)
    }
    const elementsShow = (elements, delay=1)=>{
        setTimeout(()=>{
            elements.forEach(item=>{
